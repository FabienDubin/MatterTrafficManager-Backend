...
export class ConfigController {
   ...
   * GET /api/config/:key
   * Note: Calendar configs are accessible to all authenticated users, others require admin role
   */
  async getConfig(req: Request, res: Response) {
    try {
      ...
      // Get the config to check its category
      ...
      // Check permissions: calendar category is public (authenticated), others require admin
      if (config.category !== 'calendar' && userRole !== 'admin') {
        return res.status(403).json({
          success: false,
          error: 'Admin access required for this configuration'
        });
      }
